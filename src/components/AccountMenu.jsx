...
export default function AccountMenu({ wallet, onClose, onNavigate, isMobile }) {
  ...
  const menuItems = [
    ...
    { id: 'walletPanel', label: 'Manage Wallet', icon: '⬡', desc: shortenAddr(wallet?.address) },
    { divider: true },
  ...
  return (
    <div ref={menuRef} style={{
      ...
      <div style={{ padding: '14px 16px', borderBottom: '1px solid #0d180d', display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg, #4ADE80, #22c55e)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 800, color: '#000' }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 9, color: '#4ADE80', display: 'flex', alignItems: 'center', gap: 4 }}>
            <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#4ADE80' }} /> Connected
          </div>
        </div>
      </div>
      <div style={{ padding: '6px 0' }}>
        {menuItems.map((item, i) => {
          if (item.divider) return <div key={i} style={{ height: 1, background: '#0d180d', margin: '4px 0' }} />;
          return (
            ...
            >
              <span style={{ fontSize: 16, width: 24, textAlign: 'center', color: item.danger ? '#EF4444' : '#667' }}>{item.icon}</span>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: item.danger ? '#EF4444' : '#ddd' }}>{item.label}</div>
                {item.desc && <div style={{ fontSize: 9, color: '#445', marginTop: 1 }}>{item.desc}</div>}
              </div>
            </button>
          );
        })}
